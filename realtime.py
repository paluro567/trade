...
too_close_thresh=0.005 #resistances are duplicates if within 0.5% of one another
...
def check_play(ticker, play_type, priority):
    ...
    for cur_res in resistances:
        if cur_volume>2.5*average_volume and open_price<cur_res and close_price>cur_res and cur_pct_change>1.5 and (ticker not in texted_plays):
...
def run_main():
    ...
    try:
        ...
        if alarm_plays is [] and green_plays==[]:
            raise Exception("no briefing yet")
    ...
    while True:
        ...
        print("Iteration complete - sleeping 30 seconds...")
        time.sleep(30)
        iteration+=1
