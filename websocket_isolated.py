...
async def monitor_stocks(SYMBOLS):
    ...
    while True:  # Keep trying to reconnect on failure
        # Check if it's after 8 PM EST and close the connection if needed
                        ...
                        if item.get('T') == 'b':  
                            ...
                            if len(full_min_price_history[symbol]) >= 9:
                                ...
                                if len(full_min_price_history[symbol]) >= 21:
                                    ...
                                    # Check for the crossover condition (9MA crosses above 20MA1)
                                    ...
                                    if detect_crossover(ma_9, ma_20, previous_ma_9, previous_ma_20):
                                        ...
                                        if can_send_text(symbol, last_text_sent_time):
                                            asyncio.create_task(send_text(symbol, f"SEBSOCKET: {symbol} 9MA has crossed above 20MA at price: {close_price}"))
                                            text(f"SEBSOCKET: {symbol} 9MA has crossed above 20MA at price: {close_price}")
                            ...
                            if len(simulated_prices) >= 9:
                                ...
                                if len(simulated_prices) >= 20:
                                    ...
                                    if detect_crossover(simulated_ma_9, simulated_ma_20, last_ma_9, last_ma_20):
                                        ...
                                        if can_send_text(symbol, last_text_sent_time):
                                            asyncio.create_task(send_text(symbol, f"SEBSOCKET: {symbol} 9MA is crossing below 20MA at price: {trade_price}"))
